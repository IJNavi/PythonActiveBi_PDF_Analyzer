...
class ActiveBIStyle:
    ...
    # Cores principais do site - atualizado para #0c0d64
    PRIMARY_BLUE = "#0c0d64"      # Azul escuro do header/fundo
...
class DocumentAnalyzerGUI:
    ...
    def create_main_interface(self):
        ...
        container = tk.Frame(self.root, bg=ActiveBIStyle.WHITE)
        ...
        # ==================== AJUSTE RESPONSIVO CENTRALIZADO ====================
        def resize_content(event):
            canvas_width = event.width
            content_width = min(900, canvas_width - 40)   # largura máxima do conteúdo
            x_position = max((canvas_width - content_width) // 2, 0)   # centraliza
        ...
        tk.Label(pdf_inner, text="Arquivo:", font=ActiveBIStyle.FONT_BODY,
                fg=ActiveBIStyle.PRIMARY_BLUE, bg=ActiveBIStyle.WHITE
        ).pack(side=tk.LEFT, padx=(0, 10))
        
        self.pdf_path_var = tk.StringVar()
        pdf_entry = tk.Entry(
            pdf_inner, textvariable=self.pdf_path_var,
            font=ActiveBIStyle.FONT_BODY, bg=ActiveBIStyle.BG_LIGHT,
            fg=ActiveBIStyle.PRIMARY_BLUE, relief=tk.SUNKEN,
            state='readonly', width=60
        )
        pdf_entry.pack(side=tk.LEFT, padx=(0, 10), fill=tk.X, expand=True)
        
        select_btn = tk.Button(
            pdf_inner, text="📂 SELECIONAR", font=ActiveBIStyle.FONT_BUTTON,
            bg=ActiveBIStyle.PRIMARY_BLUE, fg=ActiveBIStyle.WHITE,
            bd=0, padx=15, pady=5, cursor="hand2", command=self.select_pdf
        )
        
        def on_select_enter(e): select_btn.config(bg=ActiveBIStyle.LIGHT_BLUE)
        def on_select_leave(e): select_btn.config(bg=ActiveBIStyle.PRIMARY_BLUE)
        ...
        status_label = tk.Label(
            pdf_frame, textvariable=self.pdf_status_var,
            font=("Segoe UI", 9), fg=ActiveBIStyle.SECONDARY_BLUE,
            bg=ActiveBIStyle.WHITE
        )
        ...
        self.question_text = scrolledtext.ScrolledText(
            question_inner, height=5, wrap=tk.WORD,
        ...
        config_frame = tk.LabelFrame(
            main_frame, text=" ⚙️ Configurações da Resposta ",
            font=ActiveBIStyle.FONT_TITLE, fg=ActiveBIStyle.PRIMARY_BLUE,
            bg=ActiveBIStyle.WHITE, bd=2, relief=tk.RIDGE
        )
        ...
        tk.Label(
            config_inner, text="Nível de Detalhamento:",
            font=ActiveBIStyle.FONT_BODY, fg=ActiveBIStyle.PRIMARY_BLUE,
            bg=ActiveBIStyle.WHITE
        ...
        concise_rb = tk.Radiobutton(
            config_inner, text="📝 Conciso (2 parágrafos)",
            variable=self.response_level, value="concise",
            font=ActiveBIStyle.FONT_BODY, fg=ActiveBIStyle.PRIMARY_BLUE,
            bg=ActiveBIStyle.WHITE, selectcolor=ActiveBIStyle.WHITE,
            activebackground=ActiveBIStyle.WHITE
        )
        concise_rb.pack(side=tk.LEFT, padx=(0, 20))
        
        standard_rb = tk.Radiobutton(
            config_inner, text="📊 Padrão (1 página A4)",
            variable=self.response_level, value="standard",
            font=ActiveBIStyle.FONT_BODY, fg=ActiveBIStyle.PRIMARY_BLUE,
            bg=ActiveBIStyle.WHITE, selectcolor=ActiveBIStyle.WHITE,
            activebackground=ActiveBIStyle.WHITE
        )
        ...
        complex_rb = tk.Radiobutton(
            config_inner, text="🔬 Complexo (Análise profunda)",
            variable=self.response_level, value="complex",
            font=ActiveBIStyle.FONT_BODY, fg=ActiveBIStyle.PRIMARY_BLUE,
            bg=ActiveBIStyle.WHITE, selectcolor=ActiveBIStyle.WHITE,
            activebackground=ActiveBIStyle.WHITE
        )
        ...
        self.analyze_btn = tk.Button(
            main_frame, text="🔍 ANALISAR DOCUMENTO",
            font=("Segoe UI", 12, "bold"), bg=ActiveBIStyle.ORANGE,
            fg=ActiveBIStyle.WHITE, bd=0, padx=40, pady=12,
        ...
        response_frame = tk.LabelFrame(
            main_frame, text=" 📊 Resposta da IA ",
            font=ActiveBIStyle.FONT_TITLE, fg=ActiveBIStyle.PRIMARY_BLUE,
            bg=ActiveBIStyle.WHITE, bd=2, relief=tk.RIDGE
        )
        ...
        self.response_text = scrolledtext.ScrolledText(
            response_inner, wrap=tk.WORD, font=("Consolas", 10),
    ...
    def analyze_document(self):
        ...
        def analyze():
            ...
            try:
                ...
                context, metadata_list = self.rag_engine.retrieve_context(self.vectorstore, question, k=5)
                ...
                cost = self.cost_calculator.calculate_cost(token_info['input_tokens'], token_info['output_tokens'])
                result = {
                    "type": "text",
                    "text": response_text,
